...
namespace BAI
{
    public class BAI_Afteken1
    {
        ...
        public static void Opdr1FilterList(List<int> list)
        {
            // *** IMPLEMENTATION HERE *** //

            //Create dictionary to store all numbers as key and the amount of the number
            var SingleIntValues = new Dictionary<int, int>();

            //Loop through list and add number to dictionary as key. If number is already in dictionary plus the value
            foreach (int number in list)
            {
                if (SingleIntValues.ContainsKey(number))
                {
                    SingleIntValues[number] += 1;
                }
                else
                {
                    SingleIntValues.Add(number, 1);
                }
            }

            foreach (KeyValuePair<int, int> Number in SingleIntValues)
            {
                if (Number.Value == 1)
                {
                    //SingleIntValues.Remove(i);
                    for (int j = 0; j < SingleIntValues[Number.Key]; j++)
                    {

                        list.Remove(Number.Key);
                    }
                }
            }
        }
        ...
        public static Stack<int> Opdr3RandomNumbers(int lower, int upper, int count)
        {
            ...
            while (n < count)
            {
                ...
                {
                    dictionary.Add(randomNumber, randomNumber);
